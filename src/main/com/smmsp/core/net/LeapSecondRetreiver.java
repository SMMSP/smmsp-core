...
/**
 * Connects to HPIERS and downloads the latest bulletin-C, which
 * contains the current UTC-TAI offset (the number of leapseconds
 * added)
 * 
...
public abstract class LeapSecondRetreiver {
	...
	*/
	...
	public static int getLeapSeconds() {
		if (_downloadedSeconds > 0) { return _downloadedSeconds; }
		...
		try {
			...
			BufferedReader read = new BufferedReader(
					new InputStreamReader(conn.getDataStream()));
			...
			while((line = read.readLine()) != null){
				if(line.contains("until further notice")){
					...
					if(!m.matches()){
						throw new LeapSecondException("Parse Error in retreival");
					}
